...
function getCookie(req, key) {
    ...
    // استخراج Max-Age
    const maxAgeMatch = cookieHeader.match(new RegExp(`${key}[^;]*Max-Age=([0-9]+)`));
    const maxAge = maxAgeMatch ? parseInt(maxAgeMatch[1]) : null;

    return { value: cookies[key], maxAge };
}
...
use('POST', 'token', function (request, response) {
    readFile('./users.json', 'utf8', function (error, fileData) {
        if (error) {
            ...
            if (foundIndex >= 0) {
                let signedToken = jwt.sign(dataObject.records[foundIndex], JWT_SECRET, { expiresIn: TOKEN_TIME*60 }); 
                // Set the token as a cookie with Max-Age
                response.setHeader('Set-Cookie', `token=${signedToken}; HttpOnly; Path=/; Max-Age=`+TOKEN_TIME*60);
