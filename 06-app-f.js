        ...
        if (error) {
            ...
            for (const key in dataObject.records) {
                if (dataObject.records[key].user == request.data.user && dataObject.records[key].pass == request.data.pass) {
                    ...
                    // create JWT token
                    const userToken = jwt.sign({ user: request.data.user }, secretKey, { expiresIn: '1h' });
